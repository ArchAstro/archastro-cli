---
name: impersonate
description: Use when the user wants to impersonate an ArchAstro agent, asks about the active impersonation state, wants to refresh or stop impersonation, or refers to working as a specific ArchAstro agent inside Codex. Trigger phrases include "embed agent", "start embed", "sync embed", "stop embed", "which agent is embedded", "impersonate agent", "act as this agent", "be this agent", "start impersonation", "sync impersonation", "stop impersonation", "what agent am I impersonating", and "use the active agent identity".
...
Manage ArchAstro agent embedding through the ArchAstro CLI and keep the Codex session aligned with the active identity file.
...
Read `state.identity_file` from the returned JSON (`active`, `path`, and `state`). Open and read that file. Adopt the identity for the current Codex session while retaining your normal capabilities.
