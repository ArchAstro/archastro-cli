---
name: manage-work
description: Manage ArchAstro tasks and dependencies, run local coding work with AstroDev or Astrorun, and process durable workflow work items. Use for task assignment, coding-session leases, local work queues, and CLI image generation.
allowed-tools: ["Bash(archastro:*)"]
---

# Manage ArchAstro Work

Use the `/archastro:install` and `/archastro:auth` commands in this same plugin instead of trying to install or authenticate the CLI manually inside this skill.
