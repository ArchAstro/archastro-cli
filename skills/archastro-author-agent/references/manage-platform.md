...
Use this guide for platform administration and data management. For connecting an agent to a provider, use the [manage-integrations guide](manage-integrations.md); for agent knowledge ingestion, use the [manage-knowledge guide](manage-knowledge.md).
...
3. Create a team in the correct organization when the collaboration needs one, then manage `teammembers`. Team membership is separate from thread membership; use the [chat guide](chat.md) for conversations.
...
- For config-backed resources, use the [manage-configs guide](manage-configs.md) so local files and deployed versions stay coherent.
...
For event-triggered work, inspect `list events` and `describe event <name>` to get the payload contract. `webhooks` and `webhookevents` configure and inspect incoming provider/generic events; automation webhook signing secrets are a separate resource. Use the [build-workflow guide](build-workflow.md) to connect an event to an automation, and the [manage-integrations guide](manage-integrations.md) for provider authorization. Do not substitute a guessed event name or signature scheme for the current reference.
