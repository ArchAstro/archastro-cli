...
Manage ArchAstro agent embedding through the ArchAstro CLI and keep the current coding-agent session aligned with the active identity file.
...
Read `state.identity_file` from the returned JSON (`active`, `path`, and `state`). Open and read that file. Adopt the identity for the current current coding-agent session while retaining your normal capabilities.
