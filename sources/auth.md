...
2. **Check the installed CLI version first**:
   ...
   archastro --version
   ```
   If the command is missing, or the version is older than the resolved minimum, {{#CLAUDE_COMMAND}}tell the user to run `/archastro:install`{{/CLAUDE_COMMAND}}{{#SKILL}}instruct the user to install or upgrade `archastro`{{/SKILL}}.
